...
| 升级到新代码（git pull 之后） | `docker compose up -d --build newsflow` | ✅ | ✅（必然重建） |
...
- `WEBHOOKS_CONFIG_PATH` 或 `data/webhooks.yaml` 内容变化：`up -d` 即可（启动时会跑一次 `sync_webhooks` 把 yaml 和 DB 对齐，见 §4.6）

**需要重 build 镜像**（仅改 `.env` 不够）：

- `pyproject.toml` 改过依赖 → `docker compose build newsflow && docker compose up -d newsflow`
- 源码改动（`src/` 下任何 `.py`）→ 同上
- 切换 Python 版本 → 同上
